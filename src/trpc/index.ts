...
export const appRouter = router({
  authCallback: publicProcedure.query(async () => {
    ...
    if (!dbUser) {
      // first time user is using the application, create user in db
  ...
  createStripeSession: privateProcedure.mutation(async ({ ctx }) => {
    ...
    // if user is subscribed, send to management page
    ...
    const stripeSession = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price: PLANS.find((plan) => plan.name === "Pro")?.price.priceIds.test, // use production for production version
          quantity: 1,
        },
      ],
      metadata: {
